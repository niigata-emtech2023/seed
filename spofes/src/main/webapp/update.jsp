...
%>
	...
	<div class=flex><span id = s1>＜得点項目編集画面＞<br></span><br>
	...
	<%if(request.getParameter("task_id")!=null){ %>
		<div class=flex><span id=s3><%=request.getParameter("task_id")%></span><form action="updateconfirmation.jsp" method="post"><textarea name="update_info" cols="45" rows="4"><%=request.getParameter("task_info")%></textarea></div>
		
		
		<input type="hidden" name="task_id" value="<%=request.getParameter("task_id")%>"> 
		<input type="hidden" name="task_origin" value="<%=request.getParameter("task_info")%>"> 
